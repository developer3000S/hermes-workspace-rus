...
export const TASKS_BOARD_HELP_TEXT =
  'Перетаскивайте карточки для изменения статуса. Откройте карточку, чтобы назначить исполнителя и дату выполнения.'
...
export function TasksScreen() {
  ...
  // Загрузка исполнителей динамически из профилей + конфига
  const assigneesQuery = useQuery({
    queryKey: ASSIGNEES_KEY,
    queryFn: fetchAssignees,
    staleTime: 5 * 60_000, // профили меняются редко
  })
  ...
  // Создание карты меток из динамических исполнителей для отображения в TaskCard
  ...
  function handleDrop(e: React.DragEvent, targetColumn: TaskColumn) {
    ...
    // Гибридная автономность: если настроен проверяющий (human reviewer), только он может
    // переводить задачи в колонку 'done' — агенты могут максимум переместить в 'review'.
    if (targetColumn === 'done' && humanReviewer) {
      toast(`Только ${humanReviewer} может помечать задачи как выполненные`, { type: 'error' })
      setDraggingId(null)
      setDragOverColumn(null)
      return
    }
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs text-[var(--theme-muted)] flex-wrap">
            ...
            {stats.blocked > 0 && (
              <>
                <span className="hidden sm:inline">·</span>
                <span className="text-red-400">{stats.blocked} заблокировано</span>
              </>
            )}
            {stats.overdue > 0 && (
              <>
                  ...
                  <AnimatePresence initial={false}>
                    {colTasks.length === 0 ? (
                      ...
                      >
                        ...
                        <p className="text-xs font-medium">Нет задач</p>
                        <p className="text-[10px]">Перетащите сюда или нажмите +, чтобы добавить</p>
                      </motion.div>
                    ) : (
                      colTasks.map(task => (
                        <motion.div
