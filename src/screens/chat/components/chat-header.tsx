...
function formatMobileSessionTitle(rawTitle: string): string {
  ...
  if (
    ...
    if (candidate.toLowerCase() === 'main') return 'Основной чат'
    return `${toTitleCase(candidate)} Chat`
  }

  // Common system prompts → friendly names
  if (normalized.startsWith('read heartbeat')) return 'Основной чат'
  if (normalized.startsWith('generate daily')) return 'Ежедневный отчет'
      ...
      <div className="px-4 h-12 flex items-center">
          ...
          {isEditingTitle ? (
            ...
            >
              <button
                ...
                title="Нажмите, чтобы переключить сессию"
              >
                {activeTitle}
              </button>
        ...
        {renamingTitle ? (
          <span
            ...
            aria-label="Сохранение названия сессии"
          />
        ) : null}
        {showThinkingIndicator ? (
          <TooltipProvider>
            <TooltipRoot>
              <TooltipTrigger
                render={
                  <span
                    ...
                    aria-label="Размышление: Адаптивное"
                    role="status"
              ...
              />
              <TooltipContent side="bottom">
                Размышление: Адаптивное — Hermes рассуждает перед ответом
              </TooltipContent>
            </TooltipRoot>
          </TooltipProvider>
        ) : null}
            ...
            <TooltipRoot>
              <TooltipTrigger
                ...
                render={
                  <button
                    type="button"
                    aria-label={isStale ? 'Устарело — нажмите для синхронизации' : 'В эфире'}
                    className={cn(
              ...
              />
              <TooltipContent side="bottom">
                {isStale ? 'Устарело — нажмите для синхронизации' : 'В эфире'}
              </TooltipContent>
            </TooltipRoot>
          </TooltipProvider>
        ) : null}
        ...
        {onClear && (
          <TooltipProvider>
            <TooltipRoot>
              ...
              />
              <TooltipContent side="bottom">
                {clearConfirm ? 'Нажмите еще раз для подтверждения' : 'Очистить сессию'}
              </TooltipContent>
            </TooltipRoot>
          </TooltipProvider>
        )}
