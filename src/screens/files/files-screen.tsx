...
export const FILE_BROWSER_MODE_LABEL = 'Рабочее пространство сервера'
export const FILE_BROWSER_REMOTE_HELP =
  'Файлы загружаются с сервера рабочего пространства через /api/files, поэтому удаленные развертывания показывают файлы, созданные агентом.'
...
// ──────────────────────────────────────────────────────────────────────────────
// Построчное сравнение (без внешних библиотек)
// ──────────────────────────────────────────────────────────────────────────────
...
type DiffLine = {
  kind: DiffLineKind
  text: string
  leftNum: number | null // исходный номер строки
  rightNum: number | null // новый номер строки
}

/**
 * Очень простое сравнение на основе LCS. Создает список записей DiffLine, которые можно
 * отобразить в разделенном или едином представлении.
 */
...
// ──────────────────────────────────────────────────────────────────────────────
// Базовая подсветка синтаксиса (только классы CSS, без библиотек)
// ──────────────────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────────────────
// Модальное окно сравнения (Diff Modal)
// ──────────────────────────────────────────────────────────────────────────────
...
function DiffModal({
  ...
  // Разделение левой (исходной) и правой (новой) колонок для раздельного просмотра
    ...
    >
      <DialogContent className="max-w-5xl w-full">
        <div className="flex flex-col max-h-[85vh]">
          ...
          {/* Раздельное представление изменений */}
...
function Breadcrumb({ path }: { path: string }) {
  ...
  return (
    <div className="flex items-center gap-1 truncate text-xs text-primary-500 dark:text-neutral-400 min-w-0">
      <span className="shrink-0">рабочее пространство</span>
      {parts.map((part, i) => (
...
// ──────────────────────────────────────────────────────────────────────────────
// Панель файла — просмотр / редактирование
// Все хуки вызываются безусловно в начале функции.
// ──────────────────────────────────────────────────────────────────────────────
...
function FilePanel({ selectedEntry }: FilePanelProps) {
  ...
  // Получение информации о типе файла (безопасно независимо от null в selectedEntry)
  ...
  /** Запись на диск (вызывается после подтверждения изменений или если ничего не менялось) */
  ...
  /** Обработчик кнопки сохранения — показывает окно сравнения, если содержимое изменилось */
  const handleSave = useCallback(() => {
    if (!selectedEntry || !dirty) return
    if (editValue !== content) {
      // Сначала показать сравнение
      setShowDiff(true)
    } else {
  ...
  // ── Модальное окно сравнения (рендер всегда для согласованности хуков) ─────────────────
  ...
  // ── Состояния пустого выбора / папок ──────────────────────────────────────────────────
      ...
      <>
        ...
        <div className="flex h-full items-center justify-center text-center text-primary-400 dark:text-neutral-600">
          <div>
            ...
            <p className="text-sm">Выберите файл для предварительного просмотра или редактирования</p>
          </div>
        </div>
      </>
    )
        ...
        <div className="flex h-full items-center justify-center text-center text-primary-400 dark:text-neutral-600">
          <div>
            ...
            <p className="text-xs mt-1 opacity-70">
              Выберите файл внутри для просмотра
            </p>
          </div>
        </div>
      </>
    ...
    <div className="flex shrink-0 items-center justify-between gap-3 border-b border-primary-200 dark:border-neutral-800 px-4 py-2.5">
      ...
      <div className="flex shrink-0 items-center gap-2">
        {(isMd || isHtml) && !isImage && (
          ...
          >
            {rawMode ? (isHtml ? 'Просмотр HTML' : 'Предпросмотр') : (isHtml ? 'Код HTML' : 'Код')}
          </Button>
        )}
        {isEditable && (
          <Button
  ...
  const footer = (
    <div className="flex shrink-0 items-center gap-4 border-t border-primary-200 dark:border-neutral-800 px-4 py-1.5 text-xs text-primary-400 dark:text-neutral-500">
      ...
      {selectedEntry.modifiedAt && (
        <span>Изменен {formatDate(selectedEntry.modifiedAt)}</span>
      )}
      {dirty && (
        <span className="text-accent-500 font-medium">Несохраненные изменения</span>
      )}
    </div>
  )
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex flex-1 min-h-0 items-center justify-center overflow-auto p-6">
            {dataUrl ? (
              ...
              <div className="text-sm text-primary-400">Предпросмотр недоступен</div>
            )}
          </div>
          {footer}
        </div>
      ...
      <>
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="min-h-0 flex-1 bg-white">
            <iframe
              title={`Предпросмотр HTML: ${selectedEntry.name}`}
              srcDoc={content}
  ...
  // ── Просмотр кода (с подстветкой синтаксиса) — также код для md ───────────────
  ...
  // ── Редактируемое текстовое поле (обычный текст, код md и т.д.) ───────────────────────────
...
// ──────────────────────────────────────────────────────────────────────────────
// Основной компонент FilesScreen
// ──────────────────────────────────────────────────────────────────────────────
...
export function FilesScreen() {
  ...
  const loadTree = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          `HTTP ${res.status} — проверьте, установлена ли переменная HERMES_WORKSPACE_DIR`,
        )
      ...
      if (err instanceof Error && err.name === 'AbortError') {
        setTreeError(
          'Не удалось загрузить файлы — время ожидания запроса истекло. Проверьте, установлена ли переменная HERMES_WORKSPACE_DIR.',
        )
      } else {
  ...
  // Закрытие контекстного меню при клике снаружи / нажатии Escape
  useEffect(() => {
  ...
  return (
    <div className="flex h-full min-h-0 overflow-hidden bg-primary-50/95 dark:bg-neutral-950">
          ...
          <ScrollAreaViewport className="px-1 py-1">
            ...
            {treeLoading ? (
              <div className="px-3 py-2 text-xs text-primary-400 dark:text-neutral-500">
                Загрузка рабочего пространства сервера…
              </div>
            ) : treeError ? (
              <div className="space-y-1 px-3 py-2 text-xs text-red-500">
                <div>{treeError}</div>
                <div className="text-primary-400 dark:text-neutral-500">
                  Проверьте каталог рабочего пространства сервера или переменную HERMES_WORKSPACE_DIR; этому браузеру больше не нужен доступ к локальным папкам.
                </div>
              </div>
            ) : entries.length === 0 ? (
              <div className="px-3 py-2 text-xs text-primary-400 dark:text-neutral-500">
                Рабочее пространство сервера пусто. Файлы, созданные агентом, появятся здесь после их записи в настроенный путь рабочего пространства.
              </div>
            ) : (
              entries
      ...
      {/* ── Правая панель — просмотр / редактор файла ─────────────────────────── */}
      <main
        className={cn(
      ...
      {/* ── Диалог переименования / создания папки ─────────────────────────────── */}
      <DialogRoot
        ...
        <DialogContent>
          <div className="p-5 space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button onClick={() => void handlePromptSubmit()}>Сохранить</Button>
            </div>
          </div>
        </DialogContent>
      </DialogRoot>
          ...
          <div className="p-5 space-y-3">
            ...
            <DialogDescription>
              ...
              {deleteConfirm?.type === 'folder' &&
                ' Это приведет к удалению всего содержимого внутри.'}{' '}
