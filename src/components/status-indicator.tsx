...
async function fetchConnectionStatus(): Promise<ConnectionStatus> {
  ...
  if (!response.ok) {
    return {
      status: 'disconnected',
      label: 'Отключено',
      detail: 'Совместимый сервер не обнаружен.',
      health: false,
      chatReady: false,
...
function statusToColors(
  ...
  switch (status) {
    case 'enhanced':
      return { dot: 'bg-cyan-400', pulse: 'bg-cyan-400/40', label: 'Улучшено' }
    case 'connected':
      return {
        dot: 'bg-emerald-400',
...
function buildTooltip(
  ...
  if (data.status === 'partial') {
    if (!data.chatReady) parts.push('Отсутствует /v1/chat/completions')
    if (!data.modelConfigured) parts.push('Модель не выбрана')
  }
  if (data.status === 'enhanced') {
    parts.push('Обнаружены расширения шлюза Hermes Agent')
  }
...
/**
 * Минимальный индикатор состояния в виде точки (без текста).
 * Отображает состояние подключения к серверу: подключено, расширенный режим, частичное подключение или отключено.
 */
