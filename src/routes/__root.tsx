...
const APP_CSP = [
  ...
  "object-src 'none'",
  "form-action 'self'",
  // frame-ancestors игнорируется в meta CSP и должен отправляться как заголовок HTTP.
  ...
  head: () => ({
    meta: [
      ...
      },
      {
        name: 'description',
        content:
          'Рабочее пространство Hermes Agent для чата, инструментов, файлов, памяти и задач.',
      },
      {
        property: 'og:image',
        content: '/cover.png',
...
export function wrapInlineScript(source: string): string {
  return `(() => {\n  try {\n${source}\n  } catch (error) {\n    console.error('Ошибка встроенного скрипта загрузки', error)\n  }\n})()`
}
        ...
        <>
          ...
          <WorkspaceShell>
            <ErrorBoundary
              ...
              description="Не удалось отобразить страницу. Перезагрузите, чтобы попробовать снова."
            >
              <Outlet />
            </ErrorBoundary>
          </WorkspaceShell>
          {!isHermesWorldLandingRoute ? <SearchModal /> : null}
          {/* UsageMeter должен быть примонтирован в корне, чтобы событие OPEN_USAGE из
              плитки использования поискового модального окна имело слушателя. См. #258.
              Но публичные страницы запуска, такие как HermesWorld, не должны показывать хром использования приложения. */}
    ...
    <html lang="ru" suppressHydrationWarning>
      <head>
        ...
        <script
          ...
          // Полифилл crypto.randomUUID для небезопасных контекстов (доступ по HTTP через LAN IP)
      ...
      <body>
        <script
          ...
          (function(){
            ...
            var quips = ["Советуемся с оракулом...","Загружаем древние знания...","Прогреваем вестника...","Калибруем набор инструментов...","Призываем вашего агента...","Готовим рабочее пространство...","Наводим мосты между мирами...","Инициализируем среду выполнения агента..."];
            ...
            d.innerHTML = '<img src="/claude-avatar.webp" alt="Hermes Agent" style="width:80px;height:80px;margin-bottom:20px;border-radius:16px;filter:drop-shadow(0 8px 32px color-mix(in srgb,'+accent+' 45%, transparent))" />'
              ...
              + '<div style="font:400 14px/1 system-ui,-apple-system,sans-serif;letter-spacing:0.04em;color:'+muted+'">Рабочее пространство</div>'
            ...
            // Резервный вариант: всегда скрывать через 5с
            setTimeout(function(){ window.__dismissSplash && window.__dismissSplash(); }, 5000);
            // Быстрое скрытие: для вернувшихся пользователей
            try {
